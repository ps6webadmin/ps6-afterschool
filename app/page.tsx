...
export default async function Page() {
  ...
  return (
    <main className="min-h-screen text-slate-800" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      {/* ── Zoom Fair Callout ── */}
      {general.zoom_url && (
        <section className="bg-blue-50 border-b border-blue-100 py-5 px-6">
          <div className="max-w-3xl mx-auto flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
            <div className="flex-1">
              <p className="text-blue-900 font-semibold text-sm">🎥 Zoom Fair Recording Available</p>
              {general.zoom_password && (
                <p className="text-blue-400 text-xs mt-0.5">Password: <span className="font-mono text-blue-700">{general.zoom_password}</span></p>
              )}
            </div>
            <a href={safeHref(general.zoom_url)} target="_blank" rel="noopener noreferrer"
              className="shrink-0 inline-block text-sm bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg hover:bg-blue-800 transition-colors">
              Watch Recording →
            </a>
          </div>
        </section>
