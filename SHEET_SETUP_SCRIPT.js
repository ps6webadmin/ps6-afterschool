// ─────────────────────────────────────────────────────────────
// PS6 After School — One-click sheet setup
//
// HOW TO RUN:
//   1. Open your Google Sheet
//   2. Extensions → Apps Script
//   3. Delete any existing code, paste this entire file
//   4. Click Run → setupPS6Sheet
//   5. Approve permissions when prompted
//   6. Done — all tabs and data will be created automatically
// ─────────────────────────────────────────────────────────────

function setupPS6Sheet() {
  ...
  // ── general ──────────────────────────────────────────────────
  const general = getOrCreate('general');
  setRows(general, [
    ['key', 'value'],
  ...
  // ── key_dates ─────────────────────────────────────────────────
  const keyDates = getOrCreate('key_dates');
  setRows(keyDates, [
    ['date', 'event'],
  ...
  // ── programs ──────────────────────────────────────────────────
  const programs = getOrCreate('programs');
  setRows(programs, [
  ...
  // ── providers ─────────────────────────────────────────────────
  const providers = getOrCreate('providers');
  setRows(providers, [
  ...
  // ── offsite ───────────────────────────────────────────────────
  const offsite = getOrCreate('offsite');
  setRows(offsite, [
  ...
  // Bold the header row in each tab and auto-resize columns
  [general, keyDates, programs, providers, offsite].forEach(sheet => {
    sheet.getRange(1, 1, 1, sheet.getLastColumn()).setFontWeight('bold').setBackground('#1d4ed8').setFontColor('#ffffff');
    sheet.setFrozenRows(1);
    sheet.autoResizeColumns(1, sheet.getLastColumn());
  });

  SpreadsheetApp.getUi().alert('✅ Setup complete! All 5 tabs created and populated.\n\nRemember to share the sheet publicly:\nShare → Anyone with the link → Viewer');
}

function setRows(sheet, rows) {
